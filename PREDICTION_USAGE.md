# ⚽ 足球预测命令行工具使用指南

本文档详细介绍如何使用新创建的足球比赛预测命令行工具。

## 📁 文件说明

| 文件名 | 用途 | 平台 |
|--------|------|------|
| `predict_match.py` | 主预测脚本 | 跨平台 |
| `test_predictions.py` | 测试脚本 | 跨平台 |
| `predict_example.bat` | Windows快速启动器 | Windows |
| `predict_example.sh` | Linux/Mac快速启动器 | Linux/Mac |

## 🚀 快速开始

### 1. 基本预测命令

```bash
# 基本用法
python predict_match.py --home_team "多特蒙德" --away_team "蒙特雷" --league "世俱杯" --win_odds 1.57 --draw_odds 3.65 --lose_odds 4.60

# 带让球的预测
python predict_match.py --home_team "皇家马德里" --away_team "巴塞罗那" --league "西甲" --win_odds 2.10 --draw_odds 3.40 --lose_odds 3.60 --handicap -0.5
```

### 2. 使用快速启动器

#### Windows用户：
```cmd
# 双击运行或在命令行中执行
predict_example.bat
```

#### Linux/Mac用户：
```bash
# 首先添加执行权限
chmod +x predict_example.sh

# 运行脚本
./predict_example.sh
```

## 📋 命令行参数详解

### 必需参数

| 参数 | 说明 | 示例 |
|------|------|------|
| `--home_team` | 主队名称 | "皇家马德里" |
| `--away_team` | 客队名称 | "巴塞罗那" |
| `--league` | 联赛名称 | "西甲" |
| `--win_odds` | 主胜赔率 | 2.10 |
| `--draw_odds` | 平局赔率 | 3.40 |
| `--lose_odds` | 客胜赔率 | 3.60 |

### 可选参数

| 参数 | 说明 | 默认值 | 示例 |
|------|------|--------|------|
| `--handicap` | 让球数 | 0 | -0.5 |
| `--output_json` | 输出JSON格式 | False | - |
| `--quiet` | 静默模式 | False | - |
| `--ml_only` | 只使用ML模型 | False | - |
| `--dl_only` | 只使用DL模型 | False | - |

## 🎯 使用示例

### 示例1：世俱杯比赛
```bash
python predict_match.py \
  --home_team "多特蒙德" \
  --away_team "蒙特雷" \
  --league "世俱杯" \
  --win_odds 1.57 \
  --draw_odds 3.65 \
  --lose_odds 4.60
```

**预期输出：**
```
⚽ === 足球比赛预测工具 ===
🏠 主队: 多特蒙德
✈️  客队: 蒙特雷
🏆 联赛: 世俱杯
💰 赔率: 胜1.57 平3.65 负4.6

🔄 正在加载预测模型...
✅ 机器学习模型加载成功
✅ 深度学习模型加载成功
🤖 正在使用机器学习模型预测...
🧠 正在使用深度学习模型预测...

🤖 === 机器学习模型预测结果 ===
🏆 预测结果: 主胜

📊 胜平负概率:
   🏆 主胜: 65.2%
   🤝 平局: 21.8%
   💔 客胜: 13.0%

⚽ 总进球数: 3 (原始值: 2.8)
🎯 预测比分: 2:1
📈 详细进球: 主队2球, 客队1球

🏅 前三可能比分:
   1. 2:1 (18.5%)
   2. 3:1 (15.2%)
   3. 2:0 (12.8%)

🧠 === 深度学习模型预测结果 ===
🏆 预测结果: 主胜

📊 胜平负概率:
   🏆 主胜: 62.8%
   🤝 平局: 23.1%
   💔 客胜: 14.1%

⚽ 总进球数: 2-3 (原始值: 2.7)
🎯 预测比分: 2:1

🏅 前三可能比分:
   1. 2:1 (16.2%)
   2. 1:1 (14.8%)
   3. 2:0 (13.1%)

🔍 === 模型对比分析 ===
✅ 预测一致: 两模型都预测 主胜

📈 概率对比:
   主胜: ML 65.2% vs DL 62.8% (差异2.4%)
   平局: ML 21.8% vs DL 23.1% (差异1.3%)
   客胜: ML 13.0% vs DL 14.1% (差异1.1%)

⚽ 进球数对比:
   ML模型: 2.8球
   DL模型: 2.7球
   差异: 0.1球
   ✅ 进球数预测高度一致

💡 综合建议:
   🔥 强烈推荐: 两模型高置信度一致预测
   ⚽ 进球数: 建议考虑大球 (超过2.5球)

✅ 预测完成! 用时: 14:30:15

📌 免责声明: 预测结果仅供参考，不构成投注建议。请理性对待体育博彩。
```

### 示例2：只使用机器学习模型
```bash
python predict_match.py \
  --home_team "拜仁慕尼黑" \
  --away_team "多特蒙德" \
  --league "德甲" \
  --win_odds 1.85 \
  --draw_odds 3.80 \
  --lose_odds 4.20 \
  --ml_only
```

### 示例3：JSON输出格式
```bash
python predict_match.py \
  --home_team "巴黎圣日耳曼" \
  --away_team "马赛" \
  --league "法甲" \
  --win_odds 1.45 \
  --draw_odds 4.20 \
  --lose_odds 6.50 \
  --output_json
```

**JSON输出示例：**
```json
{
  "match_info": {
    "home_team": "巴黎圣日耳曼",
    "away_team": "马赛",
    "league": "法甲",
    "win_odds": 1.45,
    "draw_odds": 4.2,
    "lose_odds": 6.5,
    "handicap": 0
  },
  "timestamp": "2024-03-15T14:30:15.123456",
  "predictions": {
    "ml_model": {
      "result": "win",
      "probabilities": {
        "win": 0.72,
        "draw": 0.18,
        "lose": 0.10
      },
      "total_goals": "3",
      "total_goals_raw": 3.2,
      "predicted_score": "2:0",
      "top_scores": [
        ["2:0", 0.22],
        ["3:1", 0.18],
        ["2:1", 0.16]
      ],
      "goals_prediction": {
        "home": 2,
        "away": 0,
        "total": 3
      }
    },
    "dl_model": {
      "result": "win",
      "probabilities": {
        "win": 0.69,
        "draw": 0.20,
        "lose": 0.11
      },
      "total_goals": "3",
      "total_goals_raw": 3.1,
      "predicted_score": "2:1",
      "top_scores": [
        ["2:1", 0.19],
        ["3:0", 0.17],
        ["2:0", 0.15]
      ]
    }
  },
  "comparison": {
    "result_consistent": true,
    "goals_difference": 0.1
  }
}
```

## 🧪 测试工具

### 运行测试套件
```bash
# 运行所有测试用例
python test_predictions.py

# 或通过快速启动器选择选项8
```

测试套件包含以下测试用例：
1. 世俱杯比赛预测
2. 西甲比赛（带让球）
3. 英超比赛
4. 德甲比赛（仅ML模型）
5. 意甲比赛（仅DL模型）
6. 法甲比赛（JSON输出）
7. 帮助信息显示

## ⚠️ 注意事项

### 1. 模型文件要求
确保以下模型文件存在：
- `football_models.pkl` (机器学习模型)
- `football_dl_model.pth` (深度学习模型)

### 2. 依赖项检查
```bash
# 检查Python版本
python --version  # 需要 Python 3.8+

# 检查必要的包
python -c "import pandas, numpy, sklearn, torch, xgboost, joblib"
```

### 3. 输入验证
- 赔率必须大于1.0
- 球队名称和联赛名称支持中文
- 让球数可以为负数（客队让球）

### 4. 未知类别处理
如果输入的球队或联赛在训练数据中不存在，系统会：
- 自动选择最相似的已知类别
- 显示警告信息
- 降低预测置信度

## 🔧 高级用法

### 1. 批量预测
创建一个CSV文件包含多场比赛信息，然后使用循环：

```bash
# 示例：批量预测脚本
while IFS=, read -r home away league w_odds d_odds l_odds h_cap
do
    echo "预测: $home vs $away"
    python predict_match.py \
        --home_team "$home" \
        --away_team "$away" \
        --league "$league" \
        --win_odds "$w_odds" \
        --draw_odds "$d_odds" \
        --lose_odds "$l_odds" \
        --handicap "$h_cap" \
        --quiet \
        --output_json >> results.json
done < matches.csv
```

### 2. API集成
可以将脚本集成到其他系统中：

```python
import subprocess
import json

def predict_match_api(home_team, away_team, league, win_odds, draw_odds, lose_odds, handicap=0):
    cmd = [
        'python', 'predict_match.py',
        '--home_team', home_team,
        '--away_team', away_team,
        '--league', league,
        '--win_odds', str(win_odds),
        '--draw_odds', str(draw_odds),
        '--lose_odds', str(lose_odds),
        '--handicap', str(handicap),
        '--output_json',
        '--quiet'
    ]
    
    result = subprocess.run(cmd, capture_output=True, text=True)
    if result.returncode == 0:
        return json.loads(result.stdout)
    else:
        raise Exception(f"预测失败: {result.stderr}")

# 使用示例
prediction = predict_match_api(
    "皇家马德里", "巴塞罗那", "西甲",
    2.10, 3.40, 3.60, -0.5
)
print(f"预测结果: {prediction['predictions']['ml_model']['result']}")
```

## 🐛 故障排除

### 常见错误及解决方案

| 错误信息 | 原因 | 解决方案 |
|---------|------|---------|
| `ModuleNotFoundError` | 缺少依赖包 | `pip install -r requirements.txt` |
| `FileNotFoundError` | 模型文件不存在 | 先训练模型或下载预训练模型 |
| `赔率必须大于1` | 输入的赔率无效 | 检查并修正赔率数值 |
| `预测失败` | 输入数据格式错误 | 检查球队名称和数值格式 |

### 日志调试
```bash
# 启用详细输出
python predict_match.py [参数] --verbose

# 查看完整错误信息
python predict_match.py [参数] 2>&1 | tee error.log
```

## 📚 更多资源

- 查看主要功能文档: `README.md`
- 了解模型原理: `football_ml_predictor.py` 和 `football_dl_predictor.py`
- 数据获取: `football_data_crawler.py`
- 模型训练: `retrain_models.py`

---

**⚽ 祝您预测准确，投注理性！** 